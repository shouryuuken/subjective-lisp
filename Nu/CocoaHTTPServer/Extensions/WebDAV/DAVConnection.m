...
- (BOOL) supportsMethod:(NSString*)method atPath:(NSString*)path {
  ...
  if ([method isEqualToString:@"UNLOCK"]) return YES;
    ...
    if ([method isEqualToString:@"POST"] && ([path isEqualToString:@"/"] || [path hasPrefix:@"/?"])) {
        return YES;
    }
...
- (NSObject<HTTPResponse>*) httpResponseForMethod:(NSString*)method URI:(NSString*)path {
    ...
    if (([method isEqualToString:@"GET"] || [method isEqualToString:@"POST"])
        && [[[request url] relativePath] isEqualToString:@"/"]) {
        NSDictionary *params = nil;
        if ([method isEqualToString:@"GET"]) {
            params = [[self parseGetParams] retain];
            [params setValue:[self queryString] forKey:@"QUERY_STRING"];
        } else {
            params = [[self parseParams:[[[NSString alloc] initWithBytesNoCopy:[requestContentBody bytes] length:[requestContentBody length] encoding:NSUTF8StringEncoding freeWhenDone:NO] autorelease]] retain];
            NSLog(@"POST %@", params);
        }
        if (!params) {
        ...
        NSString *str = [params objectForKey:@"_"];
        [params autorelease];
        return [[[HTTPStringResponse alloc] initWithString:str] autorelease];
    }
    ...
    if (filePath) {
        if ([[[filePath stringByDeletingLastPathComponent] stringByDeletingLastPathComponent] hasPrefix:path_in_docs(@"symbols.nu")]) {
            clear_symbol_after_write([filePath lastPathComponent]);
        }
